...
import { Search, Plus, Minus, ShoppingCart, LayoutDashboard, CheckCircle2, XCircle, Bike, Eye, X, UtensilsCrossed, MapPin, Phone } from 'lucide-react';
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
                ...
                {/* Encabezado */}
                <div className="px-5 py-4 bg-muted/30 border-b border-border/60 flex items-start justify-between gap-3">
                    <div>
                ...
                {/* Info del cliente */}
                <div className="px-5 py-3 border-b border-border/40 space-y-1.5">
                ...
                {/* Ítems originales */}
                <div className="px-5 py-3 space-y-1 max-h-56 overflow-y-auto">
                        ...
                        <>
                            ...
                            {originalItems.map((item, i) => (
                                <div key={i} className="flex justify-between items-center text-sm">
                                    ...
                                    <span className="tabular-nums text-muted-foreground text-xs">{fmt(item.unit_price * item.quantity)}</span>
                                </div>
                            ))}
                            <div className="flex justify-between text-xs text-muted-foreground pt-1 border-t border-border/30 mt-1">
                    ...
                    {additionItems.length > 0 && (
                        <div className="pt-3">
                            ...
                            {additionItems.map((item, i) => (
                                <div key={i} className="flex justify-between items-center text-sm">
                                    <span className="text-foreground flex items-center gap-1.5">
                                        <span className="font-semibold tabular-nums">{item.quantity}×</span>{' '}
                                        {item.dish ?? '—'}
                                        <span className="px-1 py-px rounded-full bg-yellow-500/15 text-yellow-400 font-semibold leading-none" style={{fontSize:'9px'}}>
                                            +
                                        </span>
                                    </span>
                                    <span className="tabular-nums text-muted-foreground text-xs">{fmt(item.unit_price * item.quantity)}</span>
                            ...
                            <div className="flex justify-between text-xs text-yellow-400/80 pt-1 border-t border-border/30 mt-1">
                ...
                {/* Total */}
                <div className="px-5 py-3 bg-muted/20 border-t border-border/60 flex justify-between items-center">
...
export default function Adiciones({ tables, domicilios, categories, flash }: Props) {
    ...
    function openDetail(detail: DetailOrder) {
    ...
    // ── Fila de pedido reutilizable ────────────────────────────────────────────

    function OrderRow({
        order, isSelected, onSelect, onDetail, dim = false,
        tableNumber, isMesa,
    }: {
        ...
        return (
            <div className={`transition-colors border-b border-border/50 last:border-b-0
                ${isSelected
                    ? isClosed ? 'bg-yellow-500/8 border-l-[3px] border-l-yellow-500' : 'bg-primary/8 border-l-[3px] border-l-primary'
                    : dim ? 'border-l-[3px] border-l-transparent opacity-75 hover:opacity-100' : 'border-l-[3px] border-l-transparent'}
            `}>
                <div className="flex items-center gap-2 px-4 py-2.5">

                    {/* Área clicable de selección */}
                    <button onClick={onSelect} className="flex-1 text-left min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isSelected && (
                                <span className={`text-xs font-semibold ${isClosed ? 'text-yellow-400' : 'text-primary'}`}>
                                    {isClosed ? '← reabrirá' : '← agregando'}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground mt-0.5">
                            ...
                            <span className="font-semibold tabular-nums">{fmt(order.total)}</span>
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded-lg border border-border/60 hover:bg-muted hover:border-border transition-colors shrink-0 text-xs text-muted-foreground hover:text-foreground"
                    >
        ...
        <AppShell title="Adiciones" subtitle="Historial y adiciones de pedidos por mesa y domicilio">
            ...
            {tables.length === 0 && domicilios.length === 0 ? (
                <div className="rounded-2xl border border-border bg-card p-12 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Los pedidos del día aparecerán aquí agrupados por mesa o domicilio.</p>
                </div>
            ) : (
                <div className="grid lg:grid-cols-[320px_1fr] gap-6 items-start">
                    ...
                    {/* ══ Panel derecho: catálogo para adición ══ */}
                    {selectedCtx ? (
                        <div className="space-y-4">
                            {/* Banner pedido seleccionado */}
                            <div className="rounded-2xl border border-primary/40 bg-primary/5 px-5 py-4 flex flex-wrap items-center justify-between gap-2">
                                <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl border border-input bg-input pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                                />
                            </div>
                                        ...
                                        return (
                                            <div key={cat.id} className="rounded-2xl border border-border bg-card overflow-hidden">
                                                <div className="px-4 py-2" style={{ backgroundColor: c.bg }}>
                                                    <span className="text-xs font-bold uppercase tracking-widest" style={{ color: c.color }}>{cat.name}</span>
                                                </div>
                                                <div className="divide-y divide-border/40">
                                                    {cat.dishes.map(dish => {
                                                        const qty = cart[dish.id] ?? 0;
                                                        return (
                                                            <div key={dish.id} className={`flex items-center gap-3 px-4 py-3 transition-colors ${qty > 0 ? 'bg-primary/5' : ''}`}>
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="text-sm font-medium truncate">{dish.name}</div>
                                                                    {dish.description && (
                                                                        <div className="text-xs text-muted-foreground truncate">{dish.description}</div>
                                                                    )}
                                                                </div>
                                                                <span className="text-sm font-semibold shrink-0 tabular-nums">{fmt(dish.price)}</span>
                                                                <div className="flex items-center gap-1 shrink-0">
                                                                    <button
                                                                        onClick={() => setQty(dish.id, -1)}
                                                                        disabled={qty === 0}
                                                                        className="h-7 w-7 rounded-lg border border-border flex items-center justify-center hover:bg-muted transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                                                                    >
                            ...
                            {/* Carrito sticky */}
                            {cartCount > 0 && (
                                <div className="sticky bottom-4 rounded-2xl border border-accent/40 bg-card shadow-2xl p-4 space-y-3">
                                    <div className="flex items-center gap-2">
                                        <ShoppingCart className="h-4 w-4 text-accent shrink-0" />
                                        <span className="text-sm font-semibold flex-1">
                                            Adición · Pedido #{selectedCtx.order.id}
                                            {selectedCtx.type === 'mesa' ? ` · Mesa ${selectedCtx.table.number}` : ' · Domicilio'}
                                        </span>
                                        <span className="text-accent font-bold tabular-nums">{fmt(cartTotal)}</span>
                                    </div>
                                    <div className="space-y-1 border-t border-border/50 pt-2">
                                        {cartItems.map(({ dish, qty }) => (
                                            <div key={dish.id} className="flex justify-between text-xs text-muted-foreground">
                                                <span>{qty}× {dish.name}</span>
                                                <span className="tabular-nums">{fmt(dish.price * qty)}</span>
                                            </div>
                                        ))}
                                    ...
                                    <button
                                        ...
                                        disabled={submitting}
                                        className="w-full rounded-xl bg-primary py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                                    >

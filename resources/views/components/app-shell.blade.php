...
<div class="min-h-screen flex bg-background">
    ...
    <aside class="hidden lg:flex w-64 flex-col bg-sidebar border-r border-sidebar-border">
            ...
            @foreach ($nav as $item)
                ...
                @if (isset($item['children']))
                    ...
                    <details {{ $open ? 'open' : '' }} class="group/menu">
                        <summary class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium cursor-pointer list-none
                                           {{ $open
                    ...
                    : 'text-muted-foreground hover:bg-sidebar-accent/50 hover:text-sidebar-foreground' }}
                        ...
                        <div class="mt-0.5 ml-3 pl-4 border-l border-sidebar-border space-y-0.5 pb-1">
                            @foreach ($item['children'] as $child)
                                ...
                                : 'text-muted-foreground hover:bg-sidebar-accent/50 hover:text-sidebar-foreground' }}">
                                        <span
                                            class="h-1.5 w-1.5 rounded-full {{ $childActive ? 'bg-primary' : 'bg-muted-foreground/40' }} shrink-0"></span>
                                        {{ $child['label'] }}
                                    </a>
                            @endforeach
                        </div>
        ...
        <div class="border-t border-sidebar-border px-3 py-4">
            <a href="{{ url('/login') }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground

...
@section('content')
    <div class="min-h-screen grid lg:grid-cols-2">
        ...
        <div class="flex items-center justify-center p-8 order-2 lg:order-1">
            <div class="w-full max-w-md">
                ...
                <div class="mt-6 text-center text-sm text-muted-foreground">
                    ¿Ya tienes cuenta? <a href="{{ url('/login') }}" class="text-primary font-medium hover:underline">Inicia
                        sesión</a>
                </div>
            </div>
        </div>
        ...
        {{-- Panel decorativo --}}
        <div
            class="hidden lg:flex relative bg-gradient-hero overflow-hidden order-1 lg:order-2 p-12 flex-col justify-between">
            <div class="absolute inset-0 bg-gradient-radial opacity-40"></div>
            <div class="relative">
                <h2 class="font-display text-4xl font-bold text-primary-foreground leading-tight">Únete a +500 restaurantes
                    que ya operan con MenuGo.</h2>
                <ul class="mt-8 space-y-3">
                    @foreach(['15 días de licencia Gratis!','Migración asistida sin costo','Soporte por WhatsApp','Cancela cuando quieras'] as $item)
